...
<Dialog.Root bind:open>
	<Dialog.Portal>
		...
		<Dialog.Content
			class="fixed left-1/2 top-1/2 z-50 flex max-h-[90vh] w-[90vw] max-w-xl -translate-x-1/2 -translate-y-1/2 flex-col rounded-xl border border-ui-border bg-ui-surface p-5 shadow-2xl outline-none"
		>
			<!-- Header -->
			<div class="flex items-center justify-between border-b border-ui-border pb-3">
				<div class="flex items-center gap-2">
					...
					<div>
						...
						<p class="text-[11px] text-ui-text-muted">Drag to position, pinch or use slider to zoom</p>
					</div>
				</div>
				<Dialog.Close
			...
			<div class="mt-4 flex flex-col gap-3">
				<div class="flex items-center gap-3">
					<ZoomOut class="h-4 w-4 text-ui-text-muted shrink-0" />
					<input
						type="range"
						min="1"
						max="3"
					...
					/>
					<ZoomIn class="h-4 w-4 text-ui-text-muted shrink-0" />
					<span class="w-12 text-right text-xs font-mono text-ui-text-muted">
						{Math.round(zoom * 100)}%
					</span>
				</div>
				...
				<div class="flex items-center justify-end gap-2 border-t border-ui-border pt-3">
					<button
						type="button"
						onclick={() => (open = false)}
						class="rounded-lg border border-ui-border bg-ui-surface px-4 py-2 text-xs font-medium text-ui-text-main hover:bg-ui-bg transition-colors"
					>
						Cancel
					</button>
					<button
						...
						onclick={handleSave}
						disabled={isSaving}
						class="flex items-center gap-1.5 rounded-lg bg-slytherin px-4 py-2 text-xs font-medium text-white hover:bg-slytherin-dark disabled:opacity-50 transition-colors cursor-pointer"
					>
						<Check class="h-4 w-4" />
